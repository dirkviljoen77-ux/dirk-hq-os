...
export default function ConvertToJobPanel({ quotation, projects }: { quotation: any; projects: Array<{ id: string; name: string }> }) {
  ...
  const [businessProjectId, setBusinessProjectId] = useState(quotation.businessProjectId || "");
  ...
  async function convert() {
    ...
    const result = await convertQuotationToJob({ quotationId: quotation.id, name, startDate, endDate, businessProjectId, newProjectName });
    setBusy(false);
  ...
  return <section style={{ background: "#1E293B", border: "1px solid #334155", borderRadius: 12, padding: 20 }}>
    ...
    <div className="job-grid"><div><label>Attach job to project</label><select style={field} value={newProjectName ? "__new__" : businessProjectId} onChange={(e) => { setBusinessProjectId(e.target.value === "__new__" ? "" : e.target.value); setNewProjectName(e.target.value === "__new__" ? " " : ""); }}><option value="">Select project</option>{projects.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}<option value="__new__">+ Create new project</option></select>{newProjectName && <input style={{ ...field, marginTop: 8 }} placeholder="New project name" value={newProjectName.trimStart()} onChange={(e) => setNewProjectName(e.target.value || " ")} />}</div><div><label>Job name</label><input style={field} value={name} onChange={(e) => setName(e.target.value)} /></div><div><label>Start date</label><input type="date" style={field} value={startDate} onChange={(e) => setStartDate(e.target.value)} /></div><div><label>End date</label><input type="date" style={field} value={endDate} onChange={(e) => setEndDate(e.target.value)} /></div></div>
    {error && <p style={{ color: "#FCA5A5" }}>{error}</p>}<button disabled={busy} onClick={convert}>{busy ? "Creating job…" : "Convert to job"}</button>
    <style jsx>{`.job-grid{display:grid;grid-template-columns:2fr 2fr 1fr 1fr;gap:14px;margin:16px 0}label{display:block;color:#CBD5E1;margin-bottom:7px;font-size:13px}button{border:0;border-radius:8px;background:#16A34A;color:white;font-weight:700;padding:12px 20px;cursor:pointer}@media(max-width:900px){.job-grid{grid-template-columns:1fr 1fr}}@media(max-width:600px){.job-grid{grid-template-columns:1fr}}`}</style>
  </section>;
}
