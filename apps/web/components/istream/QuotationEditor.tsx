...
import { restoreQuotationRevision, saveQuotation, type QuotationInput } from "@/lib/actions/quotation.actions";
import ConvertToJobPanel from "@/components/istream/ConvertToJobPanel";
...
export default function QuotationEditor({ initial, clients, catalogue, projects, nextNumber }: { initial?: any; clients: Client[]; catalogue: Item[]; projects: ProjectOption[]; nextNumber: string }) {
  ...
  const [form, setForm] = useState<QuotationInput>(() => ({
    ...
    businessProjectId: initial?.businessProjectId ?? "", newProjectName: "",
  ...
  async function restoreRevision(revisionId: string) {
    if (!initial?.id || !window.confirm("Restore this earlier version? The current version will remain in the revision history.")) return;
    setSaving(true);
    const result = await restoreQuotationRevision(initial.id, revisionId);
    setSaving(false);
  ...
  return <div style={{ display: "grid", gap: 20 }}>
    <section style={{ background: "#1E293B", border: "1px solid #334155", borderRadius: 12, padding: 20 }}>
      ...
      <div className="quote-two"><div><label>Project</label><select style={inputStyle} value={form.newProjectName !== undefined && form.newProjectName !== "" ? "__new__" : form.businessProjectId ?? ""} onChange={(e) => setForm({ ...form, businessProjectId: e.target.value === "__new__" ? "" : e.target.value, newProjectName: e.target.value === "__new__" ? " " : "" })}><option value="">Not assigned yet</option>{projects.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}<option value="__new__">+ Create new project</option></select></div><div>{form.newProjectName !== undefined && form.newProjectName !== "" && <><label>New project name</label><input style={inputStyle} value={form.newProjectName.trimStart()} onChange={(e) => setForm({ ...form, newProjectName: e.target.value || " " })} /></>}</div></div>
      ...
      <div className="quote-two"><div><label>Contact person</label><input style={inputStyle} value={form.client.contactName ?? ""} onChange={(e) => setForm({ ...form, client: { ...form.client, contactName: e.target.value } })} /></div><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}><div><label>Validity (days)</label><input type="number" style={inputStyle} value={form.validDays} onChange={(e) => setForm({ ...form, validDays: Number(e.target.value) })} /></div><div><label>Status</label><select style={inputStyle} value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })}>{["DRAFT", "SENT", "ACCEPTED", "DECLINED", "EXPIRED"].map((status) => <option key={status}>{status}</option>)}</select></div></div></div>
    ...
    {initial?.revisions?.length > 0 && <section style={{ background: "#1E293B", border: "1px solid #334155", borderRadius: 12, padding: 20 }}><h3 style={{ marginTop: 0 }}>Revision history</h3><p style={{ color: "#94A3B8" }}>Current version: R{initial.revision}. Earlier saved versions can be restored safely.</p><div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>{initial.revisions.map((entry: any) => <button key={entry.id} className="secondary" disabled={saving} onClick={() => restoreRevision(entry.id)}>Restore R{entry.revision} · {new Date(entry.createdAt).toLocaleString("en-GB")}</button>)}</div></section>}
    {initial && <ConvertToJobPanel quotation={initial} projects={projects} />}
